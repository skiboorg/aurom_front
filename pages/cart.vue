...
const delItem = async (product_id)=>{
  await $api(`/api/cart/?product_id=${product_id}`,{
    method:'delete'
  })
  await get_cart()
  toast.add({ severity: 'success',summary:'Корзина', detail: 'Товар удален', life: 3000 });
}
          ...
          <div class=" p-12 border rounded-lg flex flex-col items-start gap-4">
            <div class="grid grid-cols-12  gap-3 pb-4 border-b last:pb-0 last:border-b-0 w-full" v-for="item in cart.items">
              ...
              <div class="col-span-6 md:col-span-2 flex flex-col justify-between items-end">
                ...
                <svg class="cursor-pointer" @click="delItem(item.id)" width="14" height="18" viewBox="0 0 14 18" fill="none" xmlns="http://www.w3.org/2000/svg">
